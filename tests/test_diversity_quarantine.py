...
class DiversityQuarantineTests(unittest.TestCase):
    ...
    def test_runtime_guard_fails_closed_on_wrong_diversity_owner(self) -> None:
        ...
        self.assertLess(
            source.index("_install_diversity_contracts()"),
            source.index("_export_core_api()"),
        )
    ...
    def test_boundary_document_names_authoritative_module_and_import_rule(self) -> None:
        ...
        self.assertIn("digest_diversity.py is the only authoritative production implementation", text)
        self.assertIn("Production Python modules in this repository must not import `digest_core` directly", text)
